...
const CreateEvent = ({ setView, styles }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError("❌ Server connection failed. Check backend or network.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ ...styles.heroContainer, overflowY: 'auto', padding: '40px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <div style={{ ...styles.formCard, width: '500px' }}>
          ...
          <button
            onClick={() => setView('organizer-home')}
            style={{
              background: 'none',
              border: 'none',
              color: '#3b82f6',
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ ...styles.primaryBtn, width: '100%', marginTop: '20px' }}
            >
